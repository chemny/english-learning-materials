...
| 首张就绪度门禁 | 调用图片模型前解决运行时画布、模块与风格冲突、插图文字风险和人物年龄线索 |
| 区域化文字计划 | 为标题、标签和学习内容绑定区域与出现次数，生图前移除标题拼接别名 |
...
6. 使用随附的 Python 脚本校验清单，并把文字编译成绑定区域与次数、自动移除标题拼接别名的 `generation-package.md`。
7. 运行 `generation_preflight.py`；所有硬项通过且首张就绪度达到 90/100 后才能生图。
